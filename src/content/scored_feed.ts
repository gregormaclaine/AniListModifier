...
import { log, is_error_invalid_extension } from './settings';
...
function update_status_with_score(
  ...
  switch (score_format) {
    ...
    case 'POINT_5':
      ...
      return;
    case 'POINT_3':
      mod.innerHTML += ` and gave it a &nbsp;` + FACE_SVGS[score as 1 | 2 | 3];
      return;
  }
}
...
async function update_feed_items(feed_item_els: Node[]) {
  ...
  const { scores, api_calls_left, api_calls_total } = await background_api_call(
    feed_items
  );
